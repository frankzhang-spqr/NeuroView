...
async def predict(
    ...
    results = {
        "has_tumor": False, "tumor_type": "No Tumor Detected",
        "slices_with_tumor": [], "slice_data": [], "scan_url": scan_url
    }
    ...
    for i in range(num_slices):
        ...
        bbox = get_bounding_box(mask) if has_tumor_in_slice else None

        for c, mod_name in enumerate(ordered_modalities):
            display_slice = slice_img_4_channel[:, :, c]
            ...
            results["slice_data"].append({
                "slice_index": i, "modality": mod_name, "image": img_str,
                "has_tumor": has_tumor_in_slice, "bbox": bbox
            })
